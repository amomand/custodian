...
description: Give automated playtest fixes an independent, non-blocking diegesis and arka-voice review.
...
engine:
  ...
  concurrency:
    group: "gh-aw-copilot-diegesis-${{ github.event.pull_request.number }}"

imports:
  - .github/agents/custodian-diegesis-reviewer.md
...
portable diegesis skill. Submit one `COMMENT` review even when the result is
`PASS`; the exact receipt is how the deterministic barrier knows this reviewer
finished. Do not modify files, approve, request changes, merge, or request other
reviewers.
